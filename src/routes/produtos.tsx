...
function parseLegacyMedidasToMeters(
  desc: string
): {
  altura: string;
  largura: string;
  ...
  if (parts.length === 3) {
    ...
    if (!isNaN(p0) && !isNaN(p1) && !isNaN(p2)) {
      ...
      // For legacy format:
      // "1.20×0.90×0.80" -> Largura x Profundidade x Altura
      return {
...
// Mantemos o banco exatamente como está.
// Apenas usamos tipo_movel como categoria visual.
function getCategoriaProduto(produto: Produto): string {
  const tipo = produto.tipo_movel?.trim();
...
function getCategoriaKey(categoria: string): string {
...
function ProdutosPage() {
  ...
  const filtrados = useMemo(() => {
    ...
    return produtos.filter((produto) => {
      ...
      const pertenceCategoria =
        ...
        getCategoriaKey(categoria) ===
        getCategoriaKey(categoriaSelecionada);
      ...
      return (
        ...
        cleanDescription(produto.descricao)
          .toLowerCase()
          .includes(q) ||
        (produto.tipo_movel || "")
          .toLowerCase()
        ...
        (produto.material || "")
          .toLowerCase()
          .includes(q) ||
        (produto.cor_acabamento || "")
          .toLowerCase()
          .includes(q)
      );
    });
  }, [produtos, query, categoriaSelecionada]);
  ...
  const gruposPorCategoria = useMemo(() => {
    ...
    filtrados.forEach((produto) => {
      ...
      mapa.get(key)!.push(produto);
    });

    return Array.from(mapa.entries())
      .map(([key, items]) => ({
        key,
        categoria: getCategoriaProduto(items[0]),
        produtos: items,
      }))
      .sort((a, b) =>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            ...
            placeholder="Buscar por nome, descrição, material..."
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-thin">
            ...
            <button
              ...
              onClick={() => setCategoriaSelecionada("todos")}
              className={[
                "shrink-0 h-9 px-3.5 rounded-lg border text-sm font-medium transition-all inline-flex items-center gap-2",
                categoriaSelecionada === "todos"
            ...
            {categorias.map((categoria) => {
              ...
              const ativa =
                getCategoriaKey(categoriaSelecionada) === key;

              const quantidade =
                quantidadePorCategoria.get(key) || 0;
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border bg-card p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {query || categoriaSelecionada !== "todos"
          ...
          {(query || categoriaSelecionada !== "todos") && (
            <button
              ...
              className="mt-4 h-9 px-4 inline-flex items-center gap-2 rounded-lg border text-sm font-medium hover:bg-accent"
            >
              Limpar filtros
            </button>
          )}
      ...
      {confirmar && (
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Deseja remover{" "}
              <strong>{confirmar.nome}</strong> do catálogo?
            </p>
      ...
      {selectedProduto &&
        (() => {
          const medidas = parseMedidas(
            selectedProduto.descricao
          );

          const descPura = cleanDescription(
            selectedProduto.descricao
          );
...
function ProdutoDialog({
  ...
  const [profundidade, setProfundidade] =
    useState("");
    ...
    if (open) {
      if (initial) {
        ...
        if (rawDesc.includes("===JSON_MEDIDAS===")) {
          ...
          const legacy =
            parseLegacyMedidasToMeters(rawDesc);
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {isEdit
                ? "Atualize os detalhes no catálogo"
                : "Adicione um item ao seu catálogo"}
            </p>
          </div>
        ...
        <form
          onSubmit={onSubmit}
          className="px-6 py-5 space-y-4 flex-1 overflow-y-auto md:flex-none"
        >
          {/* ─────────────────────────────────────────────────────────────────── */}
          {/* IMAGEM */}
          {/* ─────────────────────────────────────────────────────────────────── */}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                set("tipo_movel", e.target.value)
              }
              className="mt-1 w-full h-10 px-3 rounded-lg border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring/30"
              placeholder="Ex: Mesas, Cadeiras, Armários"
            />
